...
join
(
	SELECT cats.name, cats.shops_id
    from cats
    WHERE cats.name = "Murzik"
) 
as cats_name
on shops.id = cats_name.shops_id;
...
as cats_name
on shops.id = cats_name.shops_id
where cats_name.shops_id is null;
...
join 
(
	select orders.ord_datetime, orders.ord_an, orders.ord_id
    from orders
    where (orders.ord_datetime >= "2020-02-05 00:00:00" and orders.ord_datetime < "2020-02-13 00:00:00")
) as I
on analysis.an_id = I.ord_an;
